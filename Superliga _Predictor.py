def predction(file):
    ...
    home = df_merged2.loc[df_merged2['team'] == 'BRASILIA VÔLEI']
    home
    ...
    away = df_merged2.loc[df_merged2['team'] == 'VOLEI FUNVIC NATAL']
    away
    ...
    lube = [21.8,10.6,3.6,48.62,67.44,1.4,2.2]
    lube

    piacenza = [25.6,14.2,3.2,55.46,72.22,1.8,4.8]
    piacenza 



    result_italiano = lube + piacenza
    result_italiano2 = np.array(result_italiano)
    result_italiano2




    resultado_chance_ita = model.predict_proba(result_italiano2.reshape(1,-1))
    resultado_home_porcetagem = round(resultado_chance_ita[0][1] * 100, 2)
    resultado_away_porcetagem =  round(resultado_chance_ita[0][0] * 100, 2)
    print('Chances do time Lube vencer:',str(resultado_home_porcetagem)+'%\n'+'Chances do time Piacenza vencer:',str(resultado_away_porcetagem)+'%')